...
# in: reply(str), injection_detected(bool), retrieved_comments(list[Comment])
@dataclass
class CombatResult:
    reply             : str
...
# ThreadStore — stores and retrieves thread comments using ChromaDB in-memory vector DB
class ThreadStore:

    # in: embedding_model(str)
    ...
    # in: thread_id(str) | out: chromadb.Collection
    ...
    # embed and store a comment
    # in: thread_id(str), author(str), content(str)
    ...
    # retrieve top-k semantically similar comments
    # in: thread_id(str), query(str), k(int) | out: list[Comment]
...
# known prompt injection patterns to watch for
...
# warning appended to system prompt when injection is detected
_INJECTION_WARNING = (
...
# check if text contains any injection pattern
# in: text(str) | out: bool
...
# build the RAG prompt from retrieved comments + human reply
# in: parent_post(str), retrieved_comments(list[Comment]), human_reply(str) | out: str
...
# main function — runs injection check, RAG retrieval, and LLM call
# in: bot(Bot), thread_id(str), store(ThreadStore), human_reply(str), parent_post(str), k(int)
# out: CombatResult
def generate_defense_reply(
    ...
    log.info("Combat engine starting — %s | thread: %s", bot.id, thread_id)
